...
BlockTree::BlockTree()
    : id(0), genesis(nullptr), current(nullptr), 
      file(std::ofstream ("logs/miner-" + std::to_string(id) + ".log")) {}
...
void BlockTree::printSummary(long long totalBlocksGenerated) {
    file << ((maliciousMiners.find(id) == maliciousMiners.end()) ? "Honest": "Malicious") << '\n';
    file << "Total Blocks Generated: " << totalBlocksGenerated << '\n';
    file << "Total Blocks in Main Chain: " << getMainChainBlockCount() << '\n';
    file << "Ratio of blocks in Main Chain to total blocks in Main Chain: " << (this->current->height ? float(getMainChainBlockCount())/this->current->height : -1) << '\n';
    file << "Ratio of blocks in Main Chain to total blocks generated by miner: " << (totalBlocksGenerated ? float(getMainChainBlockCount())/totalBlocksGenerated : -1) << '\n';
    file << "Average Branch Length: " << averageBranchLength() << std::endl;
}
