...
std::vector<Event*> MaliciousMiner::receiveGet(GetEvent event) {
    if (event.malicious) return Miner::receiveGet(event);
        ...
        if (blockHashToID.find(event.hash) == blockHashToID.end()) {
            return std::vector<Event*>();
        }
        if (blockTree.getBlock(blockHashToID[event.hash]).owner == 0) {
...
std::vector<Event*> MaliciousMiner::receiveBroadcastPrivateChain(BroadcastPrivateChainEvent event) {
    ...
    if(receivedBroadcastPrivateChain.find(event.block_id) == receivedBroadcastPrivateChain.end()){
        ...
        // DO NOT REMOVE -- IMP CODE
        // Send all parents of lastReleasedMaliciousBlock
        // block = blockTree.getBlock(blockTree.getBlock(event.block_id).parentID);
        // while (block.id > 0) {
        //     hashes.push_back(block.hash());
        //     block = blockTree.getBlock(block.parentID);
        // }
        
        block = blockTree.getBlock(event.block_id);
        while(block.id != -1) {
