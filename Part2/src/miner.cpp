...
MaliciousMiner::MaliciousMiner(minerID_t id, int totalMiners, int txnInterval, int blkInterval, Block genesisBlock, std::vector<minerID_t> neighbors, std::vector<minerID_t> malicious_neighbors, bool eclipse)
    : Miner(id, totalMiners, txnInterval, blkInterval, genesisBlock, neighbors), eclipse(eclipse) {
    this->maliciousNeighbors = malicious_neighbors;
}

// Copy constructor for MaliciousMiner
MaliciousMiner::MaliciousMiner(const MaliciousMiner& other)
    : Miner(other), eclipse(other.eclipse), receivedBroadcastPrivateChain(other.receivedBroadcastPrivateChain) {
    this->maliciousNeighbors = other.maliciousNeighbors;
}

// Move constructor for MaliciousMiner
MaliciousMiner::MaliciousMiner(const MaliciousMiner&& other)
    : Miner(std::move(other)), eclipse(other.eclipse), receivedBroadcastPrivateChain(std::move(other.receivedBroadcastPrivateChain)) {
    this->maliciousNeighbors = other.maliciousNeighbors;
}
...
RingMaster::RingMaster(minerID_t id, int totalMiners, int txnInterval, int blkInterval, Block genesisBlock, std::vector<minerID_t> neighbors, std::vector<minerID_t> malicious_neighbors, bool eclipse)
    : MaliciousMiner(id, totalMiners, txnInterval, blkInterval, genesisBlock, neighbors, malicious_neighbors, eclipse), privateBlockTree(BlockTree()) {}
...
std::vector<Event*> Miner::getEvents(time_t currentTime, bool do_gen_block) {
...
std::vector<Event*> Miner::receiveBlock(BlockEvent event, bool malicious, bool do_gen_block) {
...
std::vector<Event*> Miner::receiveHash(HashEvent event) {
    if (gotBlock.find(event.hash) != gotBlock.end() && gotBlock[event.hash]){
        return std::vector<Event*>();
    }
...
bool Miner::confirmBlock(HashEvent event) {
    if(event.hash == processingBlock.hash()){
        ...
        if (blockTree.addBlock(processingBlock, event.timestamp) < 0) {
            std::cerr << "Miner::confirmBlock" << std::endl;
        }
...
std::vector<Event*> MaliciousMiner::receiveBlock(BlockEvent event, bool malicious, bool do_gen_block) {
    return Miner::receiveBlock(event, true, do_gen_block);
}

std::vector<Event*> MaliciousMiner::receiveGet(GetEvent event) {
    if (event.malicious) return Miner::receiveGet(event);
    else /* Honest Network */ {
        if (blockTree.getBlock(blockHashToID[event.hash]).id > 0 && blockTree.getBlock(blockHashToID[event.hash]).owner == 0) {
            // Requested block was generated by ringmaster
            if (receivedBroadcastPrivateChain.empty()) {
                return std::vector<Event*>();
            } else if (blockTree.getBlock(blockHashToID[event.hash]).id > lastReleasedMaliciousBlock){
                // Block is not supposed to be released
                // This should ideally never be invoked as any honest peer wouldn't get this block's hash if this was never supposed to be released
                return std::vector<Event*>();
            } else {
...
std::vector<Event*> MaliciousMiner::receiveBroadcastPrivateChain(BroadcastPrivateChainEvent event) {
    ...
    if(receivedBroadcastPrivateChain.find(event.block_id) == receivedBroadcastPrivateChain.end()){
        ...
        while(block.id != -1) {
            lastReleasedMaliciousBlock = block.id;
...
std::vector<Event*> RingMaster::receiveBlock(BlockEvent event, bool malicious, bool do_gen_block){
    if(event.block.owner != this->id) {
        std::vector<Event*> newEvents = Miner::receiveBlock(event, true, do_gen_block);
        auto privateChainBroadcastEvents = this->checkAndBroadcastPrivate(event.timestamp);
        newEvents.insert(newEvents.end(), privateChainBroadcastEvents.begin(), privateChainBroadcastEvents.end());
        return newEvents;
    }
    return std::vector<Event*>();
}
...
bool RingMaster::confirmBlock(HashEvent event) {
    ...
    privateBlockTree.switchToLongestChain(processingBlock, memPool);
    processingBlock = Block();
    return true;
}

std::vector<Event*> RingMaster::checkAndBroadcastPrivate(time_t currentTime, bool do_anyways) {
    if (branchBlock.id < 0) {
        return std::vector<Event*> ();
    }
    ...
    if (privateLength > 0 && (panic || do_anyways)) {
        ...
        while(privateBlock.id >= 0) {
            newEvents.push_back(new HashEvent(EventType::SEND_HASH, privateBlock.hash(), currentTime, id, id, -1, true, false));
            receivedBroadcastPrivateChain[privateBlock.id] = true;
    ...
    else if (honestLength > privateLength) {
        branchBlock = Block();
        privateBlockTree = BlockTree();
        return std::vector<Event*> ();
    }
